...
const uniqueWords = function (words) {
  ...
  let cleanWords = normalizeWords(words)

  let uniqueWords = []

  for (var i = 0 ; i < wordSplit.length ; i++) {
    if (uniqueWords.indexOf(cleanWords[i]) === -1) {
    uniqueWords.push(cleanWords[i])
    }
  }
  return uniqueWords
}
...
const wordCount = function (words, unique) {
  let cleanWords = normalizeWords(words)
  let uniqueWord = uniqueWords(words)
...
const wordFrequencies = function (words) {
  ...
  let dict = {}
  for (let i = 0 ; i < uniqueWord.length){
    push.Uni
     if(uniqueWords.indexOf(wordSplit[i]) === -1))
  }
