'use strict';

// Reuse functions wherever possible.

// 1. Write a function that takes a string argument
// splits it into an array of normalized words
// (uppercase strings without punctuation)
// and returns that array.
const normalizeWords = function (string) {
  let wordsArray = string.split(' ');
  let normalize = function (word) {
    return word.replace(/\W+/,'').toUpperCase();
  };
  wordsArray = wordsArray.map(normalize);
  return wordsArray;
};

// 2. Write a function that takes a string argument
// and returns an array of unique normalized words.
const uniqueWords = function (string) {
  let uniqueWordsAsKeys = function (array) {
    let result = {};

    array.forEach(function (word) {
      result[word] = null;
    });

    return result;
  };
  let uniqueWordsArray = Object.keys(uniqueWordsAsKeys(normalizeWords(string)));
  return uniqueWordsArray;
};

// 3. Write a function that returns the count of words in a string.
// Provid the *option* to count unique words instead of total words.
const wordCount = function (string, unique) {
  if (unique) {
    return uniqueWords(string).length;
  } else {
    return normalizeWords(string).length;
  }
};

// 4. Write a function that returns a dictionary with unique words as keys
// and a count of each word as the calue for that key
const wordFrequencies = function (string) {
  let wordFrequenciesObject = {};
  
  return
};

module.exports = {
  normalizeWords,
  uniqueWords,
  wordCount,
  wordFrequencies,
};
